...
import MaxwellGI.opto_hardware
from MaxwellGI.opto_hardware import OptoHardware
...
class OptoEnv:
    ...
    def stim_random(intensity, on_duration, timer = 5*60):
    #     t0 = time.time()
    #
    #     t_end = time.time() + timer
    #     while time.time() < t_end:
    #         time_sec = random.randint(3, 10)
    #         hz = 1/random.randint(1, 10)
    #         if verbose: print(time_sec, " Sec stim, at ", hz, " Hz")
    #         if self.verbose: print("-----------------------")
    #         stim_hz(verbose = True, hz = hz, timer = time_sec, on_duration = 20000)
    #         #sec_rest_between_stim: 10, 5, 2?
    #
    #     if self.verbose: print("-----------------------")
    #     t1 = time.time()
    #     if self.verbose: print("End: " + str(time.ctime(t1)))
    #     if self.verbose: print("total = " +  str(round(t1-t0, 2)) + " sec")
        return
...
#------------------------------------------
    # def stim_sequence(self, sequence):
    #     #Function to execute predefined sequence as list of tupples?
    #
    #     for (intensity, on_duration, off_duration) in sequence:
    #          stim_pulse(on_duration, off_duration)
        return


    # def make_pattern():
    #     #num_stims = 10, random = False, stim_on = 300, rest_between_stim = 500)
    #     pattern_list = []
    #     return maxlab.sequence()


    #def stimulate_pulse_train(num_pulses, on_duration = 300, off_duration = 500):
    ...
    # def update_stim_settings(self, intensity, on_duration = 80, off_duration = 60000, maxwell_sequence_initial_delay = 0 ):
    #     self.stim_off(self)
    #     self.opto.use_maxwell_on()
    #
    #     self.opto.set_arduino_intensity(intensity) #mW/mm^2 #self.opto.power_density_to_arduino_setting(9)
    #     self.on_duration = on_duration # MaxWell samples
    #     self.off_duration = off_duration # MaxWell sampels (20,000 == 1 sec?)
    #     self.maxwell_sequence_initial_delay = 0 #maxwell reccomens 500 samples (5ms) delay to avoid inter-sequence jitter
    #     return
