...
def sim(
    ...
    **kwargs,
    ...
    sdg, sdb = [
        ba.SpikeData.from_nest(
            rec, layer, N=len(layer), length=T + warmup_time
        ).subtime(warmup_time, ...)
    ...
    return ba.SpikeData(
        ...
        length=T,
        metadata=dict(
            opto_times=opto_times if has_opto else [],
...
f = plt.figure("Varying Optogenetic Fraction", figsize=(6.4, 6.4))
axes = plot_sds(f, sds_fraction)
for sd, ax in zip(sds_fraction, axes):
    ax.set_ylabel(f"$p_\\text{{opto}} = " f'{100*sd.metadata["p_opto"]:.0f}\\%$')
